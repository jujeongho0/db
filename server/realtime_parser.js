...
async function getTodayDistrcit() {
  const response = await axios.get(
    "https://apiv3.corona-live.com/domestic/updates.json"
  );
  const today_json = response.data;
  const today = [];
  for (const item of today_json["updates"]) {
    const all_distrcit = district_table[item["cityId"]];
    const area = all_area[item["cityId"]];
    let district = "";
    let today_info = {};
    if (all_distrcit != null) {
      for (const dist of all_distrcit) {
        if (item["guId"] == -1) {
          district = "전체";
          break;
        }
